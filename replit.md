...
**Rush in-popup ritual (QuickMatchModal):** When a Rush match is found via QuickMatchModal, the modal stays open and carries the user through the entire pre-game ritual as additional slides instead of redirecting away. Steps after `'found'` are `'rush-vote'` → `'rush-rules'` → `'rush-countdown'`, then a router push to `/battle/rush/[id]` for the question gameplay (which is still its own routed page so refresh/back/deep-link still work — the page detects `phase==='playing'` and skips its built-in voting overlay). The vote slide is cartoon-themed (2.5px black borders + 4px hard shadow + bouncy badge animations), shows up to 3 live games (`RUSH_VOTE_GAME_LIMIT`) with big tabular scores, and renders each player's pick as a colored checkmark badge — **blue (`#3b82f6`) for YOU, orange (`#fb923c`) for OPP** — so disagreement is visible at a glance (with a green "Locked!" pill when both pick the same game). The modal polls `/api/battles/rush/[matchupId]/state` every 750ms plus subscribes to the SSE channel via `getBattleStreamClient` to mirror the routed page's sync behavior. Closing the modal mid-rush hands off to `/battle/rush/[id]` so the user is never orphaned in an active matchup. Other modes (original/tournament) still show the original "Continue to Battle" button. Other navigation entry points (LiveBattlesSection inline match-found, IncomingInviteModal, NotificationsDropdown) still use `navigateToBattleStart` directly and rely on the fallback voting overlay on the rush page.
