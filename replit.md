...
Both events store `{ slotIndex, containerType }` in `event_data` and are posted via `lib/promoTracking.js` to `/api/analytics/events`. Tracking is suppressed when `localStorage.piks_analytics_opt_out === 'true'` or when the browser sets `navigator.doNotTrack === '1'`. The admin analytics page (`/admin-panel/analytics`) renders a "Promo Slot Performance" table aggregating impressions, clicks, and CTR per slot+container, fed by a new aggregation in `pages/api/admin-panel/analytics.js`.
