...
#### Promo Slot Tracking (task #337)
The dashboard's `PromoCarousel` (driven by admin-configurable slots in `pages/admin-panel/settings.js`) records lightweight engagement events into the existing `user_events` table:
- `promo_impression` fires the first time a slide becomes the active/centered slide for the current carousel mount, deduped by `(slotIndex, containerType)` so auto-advancing past the same slide repeatedly doesn't inflate counts.
- `promo_click` fires from an `onClickCapture` wrapper around each slide whenever any child element is clicked.
Both events store `{ slotIndex, containerType }` in `event_data` and are posted via `lib/promoTracking.js` to `/api/analytics/events`. Tracking is suppressed when `localStorage.piks_analytics_opt_out === 'true'` or when the browser sets `navigator.doNotTrack === '1'`. Users can flip that flag themselves via the "Don't track my activity" toggle in `/settings` Privacy section, which is also persisted per-user as `privacyPrefs.analyticsOptOut` on their profile and exposed through `useUserPreferences().analyticsOptOut`. `hooks/useEventTracking.js` checks the same `isAnalyticsOptedOut()` helper before sending page views, click events, demo/unplaced bet pings, and session start/heartbeat/end calls. The admin analytics page (`/admin-panel/analytics`) renders a "Promo Slot Performance" table aggregating impressions, clicks, and CTR per slot+container, fed by a new aggregation in `pages/api/admin-panel/analytics.js`.

#### Top-Nav Click-Trap Defenses (tasks #228, #322, #324, #345)
...
1. **Source fixes** — global modals that previously stayed mounted with only `visibility:hidden` + `pointer-events:none` (`AuthPopup`, `ChallengePopup`, the `BetSlip` persistent logo wrapper) now also apply `display: none` when closed, so the layout box is fully detached and cannot capture pointer events. All `/battle` page modals (`QuickMatchModal`, `MatchLobby`, `MatchResult`, `PlayFriendModal`, `PrivateMatchModal`, `MatchHistoryModal`, `ForfeitModal`, `ForfeitConfirmedModal`, `MessagePopup`, `IncomingInviteModal`) already `return null` when closed and therefore fully unmount. While open, each carries `data-allow-fixed-overlay="true"` on its outer overlay (or, for `ForfeitConfirmedModal`, opts out via `pointer-events:none` on the outer) so the watchdog does not misclassify the open modal as an orphan and break its controls.
2. **Orphan-overlay watchdog** — `hooks/useTopNavOrphanOverlayWatchdog.js` runs a 1.5 s interval that probes the top-nav strip with `document.elementFromPoint` and forces `pointer-events:none` on any orphan fixed-position ancestor covering the top 70 px that isn't allow-listed (`[data-topnavbar]`, `[data-betslip]`, `[data-toast-stack]`, `[role=dialog][aria-modal=true]`, `[data-scroll-lock-owner]`, `[data-allow-fixed-overlay]`). When it neutralises an offender it logs `[<page>] neutralised orphan fixed overlay covering top-nav: ...` so the offender can be identified next time the bug recurs. The hook is wired into both `pages/messenger.js` and `pages/battle.js` (the two pages with the historical repro paths) so defense is symmetric. The complementary body scroll-lock recovery lives in `hooks/useGlobalScrollLockRecovery.js` and runs globally from `pages/_app.js`. New full-screen overlays must either unmount when closed or set one of the allow-list data attributes.
