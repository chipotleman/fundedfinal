...
- **Sports Data**: Goalserve API (primary source for all data)
  - **Goalserve REST API** (primary - powers main dashboard)
    ...
    - **CRITICAL: Home/Away REVERSED**: Goalserve REST API has `hometeam`/`awayteam` REVERSED. The parser in `lib/goalserve.js` swaps them (Goalserve hometeam → our awayTeam, Goalserve awayteam → our homeTeam) so games display correctly as "Away @ Home" (away team first/top, home team second/bottom).
  ...
  - **Goalserve Inplay HTTP Feeds** (alternative real-time data - requires IP whitelisting)
    - **Home/Away Convention**: Unlike the REST API, the inplay feed uses CORRECT convention where `home` = home team and `away` = away team. NO swap needed in `lib/goalserve-inplay.js`.
