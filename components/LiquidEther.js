...
`;
        ...
        super({
          material: {
            ...
            uniforms: {
              ...
              divergence: { value: simProps.dst.texture },
              px: { value: simProps.cellScale }
            }
          },
          output: simProps.dst,
    ...
    class Output extends ShaderPass {
      ...
      update({ vel }) {
        this.plane.material.uniforms.velocity.value = vel.texture;
        super.update();
      }
    }
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        for (const key in this.fbos) {
          this.fbos[key] = new THREE.WebGLRenderTarget(fboSize.x, fboSize.y, {
      ...
      resize() {
        ...
        for (const key in this.fbos) {
          if (this.fbos[key]) this.fbos[key].setSize(width, height);
        }
      }
      update() {
        ...
        this.pressure.update({
          vel: this.options.isViscous ? this.fbos.vel_viscous1 : this.fbos.vel_1,
          pressure: pressure,
          dt: this.options.dt
        });
      }
    ...
    class WebGLManager {
      constructor(opts) {
        ...
        try {
          ...
          console.warn('LiquidEther: WebGL not available, skipping animation');
          return;
        }
        ...
        this.output.simulation = new Simulation(simOpts);
        Mouse.init(this.$wrapper);
        Mouse.autoIntensity = opts.autoIntensity || 2.0;
        Mouse.takeoverDuration = opts.takeoverDuration || 0.25;
        Mouse.onInteract = () => {
          this.lastUserInteraction = performance.now();
        };
      ...
      start() {
        ...
        const loop = () => {
          ...
          if (this.output) this.output.update({ vel: this.output.simulation.fbos.vel_0 });
        };
        loop();
      }
      pause() {
