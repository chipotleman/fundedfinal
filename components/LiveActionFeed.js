...
export default function LiveActionFeed({ comments = [], maxItems = 5, homeTeam = '', awayTeam = '' }) {
  ...
  if (!displayedComments || displayedComments.length === 0) {
    return (
      <div className="p-3 rounded-lg" style={{ backgroundColor: '#111', border: '1px solid #1a1a1a' }}>
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ backgroundColor: '#111', border: '1px solid #1a1a1a' }}>
      <div className="flex items-center gap-2 px-3 py-2" style={{ borderBottom: '1px solid #1a1a1a', backgroundColor: 'rgba(26,26,26,0.5)' }}>
        <span className="text-sm">📡</span>
        <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Live Feed</span>
      ...
      >
        {displayedComments.map((comment, index) => (
          <div
            ...
            } ${index !== displayedComments.length - 1 ? 'border-b border-[#1a1a1a]' : ''}`}
          >
