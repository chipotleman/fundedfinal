...
export function buildBattleHistoryRows({
  ...
  // Ensure every known battle is represented as a row even when the user
  // hasn't placed a bet in it yet (e.g. active Rush matches with no
  // traditional bet rows, or just-joined matchups). The filter step
  // below still hides them from tabs that don't match.
  for (const mid of Object.keys(battles)) {
    if (!allBattleBets[mid]) allBattleBets[mid] = [];
  }
