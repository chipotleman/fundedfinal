 ...
 *
 * Team data is drawn from the shared simulated-data source
 * (`lib/sportsTeams.js`) — the same canonical pools the simulated games
 * feed (`lib/simulated-games.js`) uses — so there is one place team data
 * lives and one seam to swap in real (Goalserve) rosters later. The
 * scoring model below is intentionally abstract "battle points" (not
 * real-sport scores) so cross-sport picks stay comparable and fair; that
 * is deliberately Rush-specific and does not reuse the games feed's
 * realistic per-sport score ranges.
 */
...
// Pull the short team names from the shared simulated pools so Rush and
// the games feed never drift apart.
function teamNames(pool) {
  return pool.map((t) => t.full);
}
...
function buildRoundOptions(matchupId, roundIndex) {
  ...
  return SPORT_ORDER.map((sportKey) => {
    ...
    const home = pick(rng, sport.teams);
    let away = pick(rng, sport.teams);
    let guard = 0;
    while (away === home && guard < 8) { away = pick(rng, sport.teams); guard += 1; }
