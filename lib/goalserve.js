...
function parseGoalserveGame(match, sportKey) {
  ...
  // IMPORTANT: Goalserve has home/away REVERSED in their data
  // Their "hometeam" is actually the away team (displayed first)
  // Their "awayteam" is actually the home team (displayed second)
  // We swap them here to match bet365 and standard sportsbook convention
...
function parseGoalserveOdds(match, sportKey) {
  ...
  types.forEach(type => {
    ...
    bookmakers.forEach(bm => {
      ...
      if (isMoneylineType) {
        // Goalserve labels are reversed - swap detection
      ...
      if (isSpreadType) {
        ...
        if (handicaps) {
          ...
          odds.forEach(odd => {
            if (!odd) return;
            // Goalserve labels are reversed, swap assignments
