...
const CACHE_DURATION_MS = 60 * 1000;  // 60s cache for odds - reduces API load significantly
const SCORES_CACHE_DURATION_MS = 2000; // 2s cache for score updates
...
function parseGoalserveGame(match, sportKey) {
  ...
  // Goalserve REST API uses standard convention:
  // hometeam = home team (displayed second/bottom), awayteam = away team (displayed first/top)
...
function parseGoalserveOdds(match, sportKey) {
  ...
  types.forEach(type => {
    ...
    bookmakers.forEach(bm => {
      ...
      if (isMoneylineType) {
        // Goalserve uses standard convention: Home = home team, Away = away team
        const homeTeamName = match.hometeam?.name || match.localteam?.name || '';
        const awayTeamName = match.awayteam?.name || match.visitorteam?.name || '';
        ...
        odds.forEach((odd, idx) => {
          ...
          // Detect which team the odd refers to (standard convention)
          const isHome = odd.name === '1' || odd.name === 'Home' || oddName === 'home' ||
            (homeLower && homeLower.length > 2 && (oddName.includes(homeLower) || homeLower.includes(oddName)));
          const isAway = odd.name === '2' || odd.name === 'Away' || oddName === 'away' ||
            (awayLower && awayLower.length > 2 && (oddName.includes(awayLower) || awayLower.includes(oddName)));
          
          // Standard mapping: Home odds → home_ml, Away odds → away_ml
          if (isHome && !bookmaker.home_ml) {
            bookmaker.home_ml = {
              price: parseFloat(odd.value),
              us: odd.us
            };
          } else if (isAway && !bookmaker.away_ml) {
            bookmaker.away_ml = {
              price: parseFloat(odd.value),
              us: odd.us
            };
        ...
        if (handicaps) {
          ...
          if (mainHandicap && mainHandicap.odd) {
            ...
            oddArray.forEach(odd => {
              ...
              // Standard convention: Home = home team, Away = away team
              const isHome = oddName.toLowerCase().includes('home') || 
                                 odd.name === '1' ||
                                 (match.hometeam?.name && oddName.includes(match.hometeam.name));
              const isAway = oddName.toLowerCase().includes('away') || 
                                 odd.name === '2' ||
          ...
          odds.forEach(odd => {
            if (!odd) return;
            // Standard convention: 1/Home = home team, 2/Away = away team
