...
const SCORES_CACHE_DURATION_MS = 500; // 500ms for fast score updates
...
async function getScores(sportKey) {
  ...
  // Use shorter cache duration for scores (500ms) for fast updates
  if (cached && Date.now() - cached.timestamp < SCORES_CACHE_DURATION_MS) {
    return cached.data;
  }
