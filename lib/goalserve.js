...
function parseGoalserveOdds(match, sportKey) {
  ...
  types.forEach(type => {
    ...
    bookmakers.forEach(bm => {
      ...
      if (type.value === 'Home/Away' || type.id === '1' || type.value === 'Money Line' || type.value === 'Moneyline') {
        odds.forEach(odd => {
          if (!odd) return;
...
async function getAllGamesWithOdds() {
  ...
  const results = await Promise.allSettled(
    sportEntries.map(async ([sportKey]) => {
      try {
        // Fetch odds (today+tomorrow AND yesterday for live games), and scores in parallel
        const [oddsToday, oddsYesterday, scoresGames] = await Promise.all([
          getOdds(sportKey, today, tomorrow).catch(() => []),
          getOdds(sportKey, yesterday, yesterday).catch(() => []),
          getScores(sportKey).catch(() => [])
        ]);
        
        // Combine today's and yesterday's odds
        const oddsGames = [...oddsToday];
        const todayIds = new Set(oddsToday.map(g => g.id));
        oddsYesterday.forEach(g => {
          if (!todayIds.has(g.id)) {
            oddsGames.push(g);
          }
        });
        ...
        scoresGames.forEach(game => {
          scoresMap.set(game.id, game);
        });
        
        // Merge scores into odds games where IDs match
        const mergedGames = oddsGames.map(game => {
          const scoreGame = scoresMap.get(game.id);
          if (scoreGame) {
            // Merge score data into the odds game
            return {
              ...game,
              scores: scoreGame.scores,
              status: scoreGame.status || game.status,
              isLive: scoreGame.isLive || game.isLive,
              isCompleted: scoreGame.isCompleted || game.isCompleted,
              timer: scoreGame.timer || game.timer
            };
          }
          return game;
        });
        
        // Find games from scores that are NOT in odds by ID
        // Try to match by team names for live games to preserve their odds
        const oddsIds = new Set(oddsGames.map(g => g.id));
        const additionalGames = [];
        
        scoresGames.forEach(scoreGame => {
          if (oddsIds.has(scoreGame.id)) return; // Already merged by ID
          if (!scoreGame.isCompleted && !scoreGame.isLive) return; // Skip non-completed/non-live
          
          // Try to find matching game by team names (for live games with different IDs)
          if (scoreGame.isLive) {
            const matchByTeams = oddsGames.find(og => 
              og.home_team === scoreGame.home_team && og.away_team === scoreGame.away_team
            );
            
            if (matchByTeams) {
              // Found a match by team names - merge scores into the odds game
              const idx = mergedGames.findIndex(g => g.id === matchByTeams.id);
              if (idx >= 0) {
                mergedGames[idx] = {
                  ...mergedGames[idx],
                  scores: scoreGame.scores,
          ...
          // No match found - add as completed/live game without odds
        ...
        return [...mergedGames, ...additionalGames];
      } catch (error) {
