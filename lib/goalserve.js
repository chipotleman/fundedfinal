...
function parseGoalserveOdds(match, sportKey) {
  ...
  if (!match.odds || !match.odds.type) {
    // DEBUG: Log when football games have no odds
    if (sportKey.includes('football')) {
      console.log(`[Goalserve Odds DEBUG] ${sportKey} game ${game.homeTeam} vs ${game.awayTeam} - no odds.type field`);
      console.log(`[Goalserve Odds DEBUG] odds keys: ${match.odds ? Object.keys(match.odds).join(', ') : 'no odds object'}`);
    }
    return { ...game, odds: null };
  }
  
  // Football uses different label convention - 1=Home, 2=Away directly (no swap)
  // Basketball/hockey: Goalserve reverses labels, so we need to swap
  ...
  types.forEach(type => {
    ...
    bookmakers.forEach(bm => {
      ...
      if (isMoneylineType) {
        // For basketball/hockey: Goalserve labels are REVERSED - their "1"/Home = actual away
        // For football: Labels are correct - 1=Home, 2=Away (no swap needed)
        ...
        odds.forEach((odd, idx) => {
          ...
          // For football: NO swap (1=Home, 2=Away is correct)
          // For basketball/hockey: SWAP (Goalserve reverses labels)
          if (isFootball) {
            // Football: direct mapping
            if (isGoalserveHome && !bookmaker.home_ml) {
              bookmaker.home_ml = {
                price: parseFloat(odd.value),
                us: odd.us
              };
            } else if (isGoalserveAway && !bookmaker.away_ml) {
              bookmaker.away_ml = {
                price: parseFloat(odd.value),
                us: odd.us
              };
            } else if (isTwoWayMarket && !bookmaker.home_ml && idx === 0) {
              bookmaker.home_ml = {
                price: parseFloat(odd.value),
                us: odd.us
              };
            } else if (isTwoWayMarket && !bookmaker.away_ml && idx === 1) {
              bookmaker.away_ml = {
                price: parseFloat(odd.value),
                us: odd.us
              };
        ...
        if (handicaps) {
          ...
          if (mainHandicap && mainHandicap.odd) {
            ...
            oddArray.forEach(odd => {
              ...
              // For football: NO swap (labels are correct)
              // For basketball/hockey: SWAP (Goalserve reverses labels)
              const isActualHome = isFootball ? isGoalserveHome : isGoalserveAway;
              const isActualAway = isFootball ? isGoalserveAway : isGoalserveHome;
          ...
          odds.forEach(odd => {
            if (!odd) return;
            const isGoalserveHome = odd.name === '1' || odd.name === 'Home';
            const isGoalserveAway = odd.name === '2' || odd.name === 'Away';
