  ...
  const results = await Promise.allSettled(
    sportEntries.map(async ([sportKey]) => {
      try {
        ...
        const [oddsGames, scoresGames] = await Promise.all([
        ...
        // Find completed games from scores that are NOT in odds (different IDs)
        // These are games that finished but weren't in today's schedule query
        const oddsIds = new Set(oddsGames.map(g => g.id));
        const completedOnlyGames = scoresGames
          .filter(g => !oddsIds.has(g.id) && (g.isCompleted || g.isLive))
          .map(g => ({
            ...g,
            // Add default odds structure for completed games
            odds: null
          }));
        
        
        return [...mergedGames, ...completedOnlyGames];
      } catch (error) {
