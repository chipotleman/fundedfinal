...
function parseGoalserveGame(match, sportKey) {
  ...
  // IMPORTANT: Goalserve odds feed has home/away REVERSED compared to standard convention
  // Their "hometeam" is actually the away team, and "awayteam" is actually the home team
  // Swap them here to match standard sports display (Away @ Home)
  const homeTeam = match.awayteam;  // Goalserve's awayteam = actual home team
  const awayTeam = match.hometeam;  // Goalserve's hometeam = actual away team
...
function parseGoalserveOdds(match, sportKey) {
  ...
  types.forEach(type => {
    ...
    bookmakers.forEach(bm => {
      ...
      if (isMoneylineType) {
        // IMPORTANT: Goalserve labels are reversed - their "hometeam" is our away, "awayteam" is our home
        ...
        odds.forEach((odd, idx) => {
          ...
          // Detect what Goalserve labels as "home" and "away"
        ...
        if (handicaps) {
          ...
          if (mainHandicap && mainHandicap.odd) {
            ...
            oddArray.forEach(odd => {
              ...
              // IMPORTANT: Goalserve has home/away reversed, so we swap the detection
              // Their "home" labels = our away, their "away" labels = our home
              ...
              // Swap: Goalserve home = our away, Goalserve away = our home
              const isHomeTeam = isGoalserveAway;
              const isAwayTeam = isGoalserveHome;
          ...
          odds.forEach(odd => {
            if (!odd) return;
            // IMPORTANT: Goalserve has home/away reversed, swap the assignments
            // Goalserve "1"/"Home" = our away, Goalserve "2"/"Away" = our home
