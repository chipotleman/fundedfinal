...
function parseGoalserveOdds(match, sportKey) {
  ...
  types.forEach(type => {
    ...
    bookmakers.forEach(bm => {
      ...
      if (type.value === 'Asian Handicap' || type.id === '2') {
        const handicaps = Array.isArray(type.handicap) ? type.handicap : [type.handicap];
        const mainLine = handicaps.find(h => h && h.ismain === '1') || handicaps[0];
        if (mainLine) {
          ...
          lineOdds.forEach(odd => {
            ...
            if (odd.name === '1') {
              bookmaker.home_spread = {
                point: parseFloat(odd.handicap || mainLine.name),
                price: parseFloat(odd.value),
                us: odd.us
              };
            } else if (odd.name === '2') {
              bookmaker.away_spread = {
                point: parseFloat(odd.handicap || mainLine.name) * -1,
                price: parseFloat(odd.value),
                us: odd.us
              };
            }
      ...
      if (type.value === 'Over/Under' || type.id === '3') {
        const totals = Array.isArray(type.total) ? type.total : [type.total];
...
async function getAllGamesWithOdds() {
  ...
  for (const [sportKey, sportInfo] of Object.entries(SUPPORTED_SPORTS)) {
    try {
      const games = await getOdds(sportKey, today, tomorrow);
      allGames.push(...games);
    } catch (error) {
