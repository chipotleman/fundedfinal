...
async function getOdds(sportKey, date1 = null, date2 = null) {
  ...
  try {
    ...
    if (data.shedules) {
      // Handle tournament which can be a single object or an array (NFL has Pre Season, Regular Season, Post Season)
      const tournaments = Array.isArray(data.shedules.tournament) 
        ? data.shedules.tournament 
        : [data.shedules.tournament].filter(Boolean);
      
      tournaments.forEach(tournament => {
        if (tournament && tournament.week) {
          const weeks = Array.isArray(tournament.week) 
            ? tournament.week 
            : [tournament.week];
          weeks.forEach(week => {
            if (week.matches) {
