...
const MAX_POLL_INTERVAL = 300 * 1000;
...
async function fetchScheduledGames() {
  try {
    ...
    console.log(`[Schedule Cache] Cached ${formattedGames.length} scheduled games in ${elapsed}ms`);
    
    if (consecutiveErrors > 0) {
      console.log('[Schedule Cache] API recovered, resetting poll interval');
      consecutiveErrors = 0;
      resetSchedulePollInterval();
    }
    ...
    if (consecutiveErrors <= 3 || consecutiveErrors % 10 === 0) {
      console.error(`[Schedule Cache] Error fetching games (${consecutiveErrors} consecutive):`, error.message);
    }
    if (consecutiveErrors >= 3) {
      adjustSchedulePollInterval();
    }
    return cachedGames;
  }
}

function adjustSchedulePollInterval() {
  const newInterval = Math.min(MAX_POLL_INTERVAL, POLL_INTERVAL * Math.pow(2, Math.min(consecutiveErrors - 3, 3)));
  if (currentPollInterval !== newInterval && isPolling) {
    currentPollInterval = newInterval;
    if (consecutiveErrors === 3 || consecutiveErrors % 10 === 0) {
      console.warn(`[Schedule Cache] Backing off to ${Math.round(newInterval / 1000)}s interval`);
    }
    if (pollInterval) {
      clearInterval(pollInterval);
    }
    pollInterval = setInterval(fetchScheduledGames, currentPollInterval);
  }
}

function resetSchedulePollInterval() {
  if (currentPollInterval !== POLL_INTERVAL && isPolling) {
