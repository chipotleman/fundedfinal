...
// Faster polling for quicker odds updates (configurable via env var, default 15s)
const POLL_INTERVAL = parseInt(process.env.SCHEDULE_POLL_INTERVAL_MS || '15000', 10);
...
async function fetchScheduledGames() {
  try {
    ...
    // Log sport breakdown for debugging (especially NCAAB)
    const sportCounts = {};
    formattedGames.forEach(g => {
      const sport = g.sportName || 'Unknown';
    ...
    console.log(`[Schedule Cache] Sport breakdown:`, JSON.stringify(sportCounts));
