...
// Canadian Basketball Super League (BSL) / NBL Canada
const CANADA_BSL_TEAMS = new Set([
  'Lake Erie Soldiers', 'KW Titans', 'Kitchener-Waterloo Titans', 
  'Sudbury Five', 'London Lightning', 'Windsor Express', 'Niagara River Lions',
  'Brampton Honey Badgers', 'Ottawa Blackjacks', 'Saskatchewan Rattlers',
  'Edmonton Stingers', 'Fraser Valley Bandits', 'Scarborough Shooting Stars',
  'Calgary Surge', 'Montreal Alliance', 'Vancouver Bandits',
  'Lake Erie', 'Titans', 'Soldiers', 'Lightning', 'Express', 'River Lions',
  'Honey Badgers', 'Blackjacks', 'Rattlers', 'Stingers', 'Bandits', 'Shooting Stars', 'Surge', 'Alliance'
]);

// Common US college team names that shouldn't match pro teams
const COLLEGE_TEAM_NAMES = new Set([
  'Milwaukee', 'Wisconsin', 'Michigan', 'Ohio', 'Indiana', 'Illinois', 'Iowa',
  'Minnesota', 'Nebraska', 'Northwestern', 'Purdue', 'Rutgers', 'Maryland',
  'Penn State', 'MSU', 'OSU', 'PSU', 'UM', 'UW', 'UI', 'UK', 'UNC', 'UCLA', 'USC',
  'Duke', 'Kentucky', 'Kansas', 'Gonzaga', 'Villanova', 'Baylor', 'Auburn',
  'Tennessee', 'Alabama', 'Houston', 'Arkansas', 'Marquette', 'Creighton',
  'Xavier', 'UConn', 'Florida', 'Oregon', 'Arizona', 'Colorado', 'Utah',
  'Stanford', 'Syracuse', 'Louisville', 'Virginia', 'Wake Forest', 'NC State',
  'Georgia', 'LSU', 'Ole Miss', 'Mississippi State', 'Vanderbilt', 'South Carolina',
  'Missouri', 'Texas', 'Oklahoma', 'TCU', 'Texas Tech', 'Boise State', 'San Diego State',
  'Nevada', 'New Mexico', 'UNLV', 'Wyoming', 'Air Force', 'Army', 'Navy',
  'Clemson', 'Florida State', 'Miami', 'Pittsburgh', 'Boston College', 'Notre Dame',
  'Bowling Green', 'Miami Ohio', 'Miami OH', 'Miami (OH)', 'Miami (Ohio)',
  'Kent State', 'Akron', 'Toledo', 'Ball State', 'Buffalo', 'Ohio', 'Northern Illinois',
  'Western Michigan', 'Eastern Michigan', 'Central Michigan',
  'Appalachian State', 'Coastal Carolina', 'Georgia Southern', 'Georgia State',
  'Louisiana', 'South Alabama', 'Texas State', 'Troy', 'Arkansas State',
  'James Madison', 'Marshall', 'Old Dominion', 'Southern Miss',
  'Charlotte', 'FAU', 'FIU', 'North Texas', 'Rice', 'UTEP', 'UTSA', 'UAB',
  'Tulane', 'Tulsa', 'SMU', 'Memphis', 'East Carolina', 'Temple', 'Navy',
  'BYU', 'Cincinnati', 'UCF', 'West Virginia',
  'San Jose State', 'Fresno State', 'Hawaii', 'UNLV', 'Utah State',
  'Air Force', 'Colorado State', 'Wyoming', 'New Mexico', 'Nevada',
  'Wichita State', 'VCU', 'Dayton', 'Saint Louis', 'George Mason', 'Richmond',
  'Davidson', 'La Salle', 'Fordham', 'Duquesne', 'George Washington',
  'Saint Josephs', 'Rhode Island', 'UMass', 'Saint Bonaventure'
]);
...
// eSports suffix patterns - pro team name followed by weird suffix
const ESPORTS_SUFFIX_PATTERNS = [
  /\((?!W\)|Women|FL|OH|CA|TX|NY|NJ|PA|IL|LA|GA)[A-Z][a-z]+\)$/,
  /\(ANIMAL\)/i, /\(CATS?\)/i, /\(DOGS?\)/i, /\(LIONS?\)/i, /\(TIGERS?\)/i,
  /\(BEARS?\)/i, /\(WOLVES?\)/i, /\(EAGLES?\)/i, /\(HAWKS?\)/i,
  /\(CYBER\)/i, /\(VIRTUAL\)/i, /\(SIM\)/i, /\(GAMING\)/i,
  /\(RED\)/i, /\(BLUE\)/i, /\(GREEN\)/i, /\(GOLD\)/i, /\(BLACK\)/i, /\(WHITE\)/i
];
...
function hasEsportsSuffix(name) {
  ...
  const allowedSuffixes = ['W', 'Women', 'FL', 'OH', 'CA', 'TX', 'NY', 'NJ', 'PA', 'IL', 'LA', 'GA', 'Ohio', 'Florida'];
  if (allowedSuffixes.some(a => a.toLowerCase() === suffix.toLowerCase())) {
    return false;
  }
  return true;
}
...
function isEsportsTeam(name) {
  if (!name) return false;
  if (hasEsportsSuffix(name)) return true;
...
function isDirectCollegeName(name) {
  if (!name) return false;
  const normalized = normalizeTeamName(name).trim();
  if (COLLEGE_TEAM_NAMES.has(normalized)) return true;
  const words = normalized.split(' ');
  if (words.length >= 1 && COLLEGE_TEAM_NAMES.has(words[0])) return true;
  if (words.length >= 2) {
    const twoWords = words.slice(0, 2).join(' ');
    if (COLLEGE_TEAM_NAMES.has(twoWords)) return true;
  }
  for (const college of COLLEGE_TEAM_NAMES) {
    if (normalized.toLowerCase().startsWith(college.toLowerCase() + ' ')) return true;
    if (normalized.toLowerCase() === college.toLowerCase()) return true;
  }
  return false;
}
...
function checkTeamInSet(teamName, teamSet) {
  ...
  for (const team of teamSet) {
    ...
    if (normalized.toLowerCase().startsWith(team.toLowerCase() + ' ')) return true;
    if (team.toLowerCase().startsWith(normalized.toLowerCase() + ' ')) return true;
  }
  return false;
}
...
export function inferLeague(homeTeam, awayTeam, sport = 'unknown') {
  ...
  if (sport === 'basketball') {
    const isDirectCollege = isDirectCollegeName(home) || isDirectCollegeName(away);
    ...
    if (checkTeamInSet(home, CANADA_BSL_TEAMS) || checkTeamInSet(away, CANADA_BSL_TEAMS)) {
      return 'Canada BSL';
    }
