...
// The bus above is an in-process Node EventEmitter. It only works when the
// whole app runs as ONE always-on instance (Replit Reserved VM / a single
// VM). On any horizontally-scaled or serverless deployment (Replit
// Autoscale, Vercel functions, multiple cluster workers) each process gets
// its OWN EventEmitter, so an event published by the process that handled
// the writer's request never reaches a reader connected to a different
// process — invites/PnL silently degrade to the ~7s client poll.
//
// This guard makes the constraint loud at boot. It can't count peer
// instances directly (Replit doesn't expose a count), but it surfaces the
// one red flag we CAN see — multiple cluster workers in this process group —
// and always prints the deployment context so a misconfiguration shows up
// in the logs. Safe to call more than once; it only logs on the first call.
let busGuardLogged = false;
function assertSingleInstanceBus() {
  if (busGuardLogged) return;
  busGuardLogged = true;
  const workers = parseInt(process.env.WEB_CONCURRENCY || '', 10);
  const isDeployment =
    process.env.REPLIT_DEPLOYMENT === '1' || process.env.REPLIT_DEPLOYMENT === 'true';
  const context = isDeployment
    ? `deployment ${process.env.REPLIT_DEPLOYMENT_ID || '(id unknown)'}`
    : 'dev';
  if (Number.isFinite(workers) && workers > 1) {
    console.warn(
      `[battle-events] WARNING: WEB_CONCURRENCY=${workers} spawns multiple Node ` +
        `processes, but the real-time SSE bus is in-process. Cross-process battle ` +
        `events (invites, live PnL, forfeits) will be dropped and clients will fall ` +
        `back to polling. Run a single instance (Reserved VM) or move the bus to ` +
        `Postgres LISTEN/NOTIFY or Redis pub/sub.`
    );
  }
  console.log(
    `[battle-events] In-process SSE event bus active (${context}). Requires a ` +
      `single always-on instance — do not horizontally scale (Autoscale / multiple ` +
      `workers) without a shared pub/sub backend.`
  );
}
