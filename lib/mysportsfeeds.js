...
async function fetchOddsForDate(season, dateStr) {
  ...
  try {
    ...
    if (oddsResponse.ok) {
      ...
      console.log(`Odds endpoint 403 response body: ${errorBody}`);
      oddsStatus.message = `403 Forbidden - ${errorBody || 'Odds feed not enabled for this API key. Contact MySportsFeeds support.'}`;
      oddsStatus.rawError = errorBody;
    } else {
      ...
      if (response.ok) {
        ...
        if (i === 0) {
          ...
          if (oddsStatus?.code === 403) {
            debugInfo.apiTier = `403 Access Denied - Odds feed not enabled for API key. ${oddsStatus.rawError ? 'API says: ' + oddsStatus.rawError : 'Contact MySportsFeeds support to enable odds access.'}`;
