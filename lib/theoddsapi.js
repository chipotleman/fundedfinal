...
async function fetchSportOdds(sportKey) {
  ...
  // Only fetch games for today to minimize credit usage
  const now = new Date();
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const todayEnd = new Date(todayStart);
  todayEnd.setDate(todayEnd.getDate() + 1);
  todayEnd.setSeconds(todayEnd.getSeconds() - 1); // End at 23:59:59
  
  // Format as ISO 8601 without milliseconds (YYYY-MM-DDTHH:MM:SSZ)
  const formatISO = (date) => date.toISOString().split('.')[0] + 'Z';
  const commenceTimeFrom = formatISO(todayStart);
  const commenceTimeTo = formatISO(todayEnd);
