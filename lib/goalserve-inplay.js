...
class GoalserveInplayService {
  ...
  async fetchNFLScheduleOdds() {
    ...
    const url = `${GOALSERVE_BASE_URL}/${apiKey}/football/nfl-shedule?date1=${today}&date2=${today}&showodds=1&json=1`;
    ...
    try {
      ...
      const matches = this.extractNFLMatches(data);
  ...
  extractNFLMatches(data) {
    const matches = [];
    
    if (!data?.shedules) return matches;
    
    if (data.shedules.tournament?.week) {
        ...
        if (week.matches) {
          ...
          for (const wm of weekMatches) {
            if (wm.match) {
              ...
              matches.push(...dayMatches.filter(m => m && m.hometeam));
            }
          }
        }
      }
    ...
    if (data.shedules.matches?.match) {
      const dayMatches = Array.isArray(data.shedules.matches.match) 
        ? data.shedules.matches.match 
        : [data.shedules.matches.match];
      matches.push(...dayMatches.filter(m => m && m.hometeam));
    }
  ...
  normalizeNFLScheduleEvent(match) {
    ...
    const homeTeam = formatTeamName(match.awayteam?.name || '');
    const awayTeam = formatTeamName(match.hometeam?.name || '');
    
    const homeScore = parseInt(match.awayscore || match.away_score || 0);
    const awayScore = parseInt(match.homescore || match.home_score || 0);
    
    let status = 'not_started';
    if (match.status === 'Final' || match.status === 'F' || match.status === 'FT') {
      status = 'ended';
    } else if (match.timer || match.status?.includes('Q') || match.status === 'Live') {
      status = 'live';
    }
    
    const odds = this.parseNFLScheduleOdds(match.odds);
