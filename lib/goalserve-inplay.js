...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    // Goalserve provides xy coordinates in various formats
    let xy = event.xy || event.extra?.xy || event.info?.xy || null;
    
    // Extract possession info (which team has the ball/puck)
    // Goalserve uses "attack" or "possession" fields
    let possession = event.possession || event.extra?.possession || 
                     event.attack || event.extra?.attack ||
                     event.info?.possession || event.info?.attack || null;
    ...
    if (possession === '1') possession = 'away';
    else if (possession === '2') possession = 'home';
