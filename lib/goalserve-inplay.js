...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    // Debug: Log raw event structure for basketball/hockey to find all available fields
    if ((sport === 'basketball' || sport === 'hockey') && !this._rawEventLogged) {
      this._rawEventLogged = true;
      const teamName = event.home?.name || event.away?.name || 'unknown';
      console.log(`[RAW ${sport.toUpperCase()} EVENT] ${teamName}`, JSON.stringify(event, null, 2));
    }
    ...
    // Temporary production logging for hockey to identify possession fields
    if (sport === 'hockey' && !this._hockeyPossLoggedOnce) {
      this._hockeyPossLoggedOnce = true;
      console.log(`[Hockey Possession Debug] ${homeTeam} vs ${awayTeam}`);
      console.log(`  info.attack: ${event.info?.attack}`);
      console.log(`  info.possession: ${event.info?.possession}`);
      console.log(`  info.ball_pos: ${event.info?.ball_pos}`);
      console.log(`  info.state: ${event.info?.state}`);
      console.log(`  team_info.home.Serve: ${event.team_info?.home?.Serve}`);
      console.log(`  team_info.away.Serve: ${event.team_info?.away?.Serve}`);
      console.log(`  extra keys: ${event.extra ? Object.keys(event.extra).join(',') : 'none'}`);
    }
    ...
    // Check info.state field - may contain zone/possession indicators
    // Some state codes indicate which zone the puck is in
    if (!possession && event.info?.state) {
      const state = String(event.info.state);
      // State codes vary by sport - common patterns:
      // High numbers might indicate offensive zone, low numbers defensive
  ...
  startPolling(sports = null) {
    ...
    const poll = async () => {
      ...
      // Fetch all sports in PARALLEL for true 1-second updates
      await Promise.all(
      ...
      if (pollDuration > 2000) {
        console.log(`[Goalserve Inplay] Poll cycle took ${pollDuration}ms`);
      }
