  ...
  async fetchNFLScheduleOdds() {
    ...
    try {
      ...
      const liveMatches = matches.filter(m => {
        ...
        return status.includes('q') || status.includes('progress') || status === 'live' || status.includes('half');
      });
      
      console.log(`[NFL Schedule Odds] Fetched ${matches.length} NFL games for ${today}, ${liveMatches.length} live`);
