...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    if (event.timer) {
      ...
      // Format: "Q2 04:32" or "OT 02:15" or just the time remaining
      let period = '';
      if (timer.q) {
        period = `Q${timer.q}`;
      } else if (timer.period) {
        // Clean up period names - fix typos like "Overtimer" -> "OVERTIME"
    ...
    // Also check extra.period or info for period info
    if (!displayClock && event.extra?.period) {
      let extraPeriod = event.extra.period;
      // Fix typos in extra.period too
      const periodLower = extraPeriod.toLowerCase();
      if (periodLower.includes('overtime') || periodLower.includes('overtimer') || periodLower === 'ot') {
        extraPeriod = 'OVERTIME';
      }
      displayClock = extraPeriod;
    }
    if (!displayClock && event.info?.period) {
      let infoPeriod = event.info.period;
      const periodLower = infoPeriod.toLowerCase();
      if (periodLower.includes('overtime') || periodLower.includes('overtimer') || periodLower === 'ot') {
        infoPeriod = 'OVERTIME';
      }
      displayClock = infoPeriod;
    }
