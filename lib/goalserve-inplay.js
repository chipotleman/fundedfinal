...
// Get API key for inplay feeds with odds
const GOALSERVE_API_KEY = process.env.GOALSERVE_API_KEY || '';

// Build inplay feed URL with odds parameters
const buildInplayUrl = (sport) => {
  const baseUrls = {
  ...
  const base = baseUrls[sport];
  if (!base) return null;
  // Add withOdds=1, json=1, and API key to get odds data
  return `${base}?withOdds=1&json=1&key=${GOALSERVE_API_KEY}`;
};

const INPLAY_FEEDS = {
  soccer: buildInplayUrl('soccer'),
  basketball: buildInplayUrl('basketball'),
  tennis: buildInplayUrl('tennis'),
  volleyball: buildInplayUrl('volleyball'),
  amfootball: buildInplayUrl('amfootball'),
  hockey: buildInplayUrl('hockey'),
  baseball: buildInplayUrl('baseball'),
  esports: buildInplayUrl('esports')
};
  ...
  normalizeOdds(odds, sport = 'unknown', homeTeam = '', awayTeam = '') {
    ...
    for (const market of markets) {
      ...
      if ((marketName.includes('money line') || marketName.includes('home/away') || marketName === 'home away' || marketName === 'match winner') && !normalized.moneyline) {
        ...
        // First check if we have Home/Away or 1/2 labeled participants (need swap)
        // Goalserve uses "Home"/"Away" or "1"/"2" for non-football sports
        ...
        if (hasHomeAwayLabels) {
          // Standard Home/Away labels - Goalserve has them REVERSED
          ...
          // Team name labels (football) - match by team name, no swap needed
      ...
      if (isSpreadMarket && (!normalized.spread || (isPreferredSpreadMarket && !normalized._spreadIsPreferred))) {
        // Check if we have Home/Away or 1/2 labeled participants (need swap)
        // Goalserve uses "Home"/"Away" or "1"/"2" for non-football sports
        const hasHomeAwayLabels = participants.some(p => 
          p.name === 'Home' || p.name === 'Away' || p.short_name === 'Home' || p.short_name === 'Away' ||
          p.name === '1' || p.name === '2'
        ...
        if (hasHomeAwayLabels) {
          ...
          homeParticipants = goalserveAwayParticipants;  // Goalserve "Away" = actual home
          awayParticipants = goalserveHomeParticipants;  // Goalserve "Home" = actual away
        } else {
          // Team name labels (football) - match by team name, no swap needed
