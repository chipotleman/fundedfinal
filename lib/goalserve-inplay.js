...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    // DEBUG: Log position/possession fields for first event per sport to identify correct fields
    if (sport === 'hockey' && !this._debugLoggedHockey) {
      this._debugLoggedHockey = true;
      console.log(`[Goalserve Debug] Hockey event raw fields for ${homeTeam} vs ${awayTeam}:`);
      console.log(`  info keys: ${event.info ? Object.keys(event.info).join(', ') : 'none'}`);
      ...
      console.log(`  team_info.home keys: ${event.team_info?.home ? Object.keys(event.team_info.home).join(', ') : 'none'}`);
      console.log(`  Full info object:`, JSON.stringify(event.info, null, 2));
    }
    ...
    // - state field may indicate zone/possession
    let possession = event.info?.attack || event.info?.possession || 
                     ...
                     event.attack || event.extra?.attack ||
                     event.team_info?.home?.Serve || event.team_info?.away?.Serve ||
                     null;
    
    // Check state field - may contain possession/zone info
    if (!possession && event.info?.state) {
      const state = event.info.state;
      // State codes may indicate field position/possession
      // Log for debugging
      if (state && state !== '') {
        console.log(`[Goalserve Inplay] Event ${eventId} has state: ${state}`);
      }
    }
