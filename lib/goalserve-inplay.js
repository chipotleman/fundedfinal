...
class GoalserveInplayService {
  constructor() {
    ...
    this.circuitBreakerUntil = new Map();
  }
  ...
  async fetchFeed(sport) {
    ...
    const cbUntil = this.circuitBreakerUntil.get(sport) || 0;
    if (Date.now() < cbUntil) {
    ...
    try {
      ...
      this.consecutiveErrors.set(sport, 0);
      this.circuitBreakerUntil.delete(sport);
      
      return data;
    } catch (error) {
      const errCount = (this.consecutiveErrors.get(sport) || 0) + 1;
      this.consecutiveErrors.set(sport, errCount);
      this.errors.set(sport, error.message);

      if (errCount >= 5) {
        const backoffMs = Math.min(300000, 10000 * Math.pow(2, Math.min(errCount - 5, 5)));
        this.circuitBreakerUntil.set(sport, Date.now() + backoffMs);
        if (errCount === 5 || errCount % 20 === 0) {
          console.warn(`[Goalserve Inplay] Circuit breaker OPEN for ${sport}: ${errCount} consecutive errors, backing off ${Math.round(backoffMs / 1000)}s`);
        }
      } else {
