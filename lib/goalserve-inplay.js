...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    // Parse odds from gzip feed event data
    const parsedOdds = this.normalizeOdds(event.odds, sport, homeTeam, awayTeam);
    
    // Debug: Log when football has/doesn't have odds in gzip feed
    if (sport === 'amfootball') {
      if (event.odds) {
        console.log(`[Goalserve Inplay DEBUG] ${sport} gzip event has odds:`, JSON.stringify(event.odds).substring(0, 200));
      } else {
        console.log(`[Goalserve Inplay DEBUG] ${sport} gzip event NO ODDS for ${homeTeam} vs ${awayTeam}`);
      }
    }
