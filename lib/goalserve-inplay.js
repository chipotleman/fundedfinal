...
class GoalserveInplayService {
  ...
  normalizeOdds(odds) {
    ...
    // Log all market names for debugging (first event only)
    const marketNames = markets.map(m => m?.name).filter(Boolean);
    if (marketNames.length > 0) {
      console.log('[Inplay Odds] Available markets:', marketNames.slice(0, 10).join(', '));
    }
    ...
    for (const market of markets) {
      ...
      const isFullGameTotal = (marketName === 'over/under' || marketName === 'total' || marketName === 'totals' || 
                               ...
                               marketName === 'game lines total' || marketName.includes('total') || 
                               marketName.includes('o/u') || marketName.includes('over/under'));
