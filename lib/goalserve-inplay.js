...
// Helper to format team names with consistent title case
function formatTeamName(name) {
  if (!name) return '';
  
  // Only these specific abbreviations stay uppercase (sports-related)
  const keepUppercase = ['FC', 'SC', 'AC', 'BC', 'KC', 'U23', 'U21', 'U19', 'U17', 'II', 'III', 'IV', 'USA', 'UK'];
  
  // Split on spaces and format each word
  return name.split(' ').map(word => {
    const upperWord = word.toUpperCase();
    // Keep only specific known sport abbreviations uppercase
