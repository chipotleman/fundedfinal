...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    // IMPORTANT: Goalserve inplay feed has home/away REVERSED
    // Their "home" field = actual away team, their "away" field = actual home team
    // Swap them here to match standard convention (Away @ Home)
    let homeTeam = event.away?.name || event.away || '';  // Goalserve away = actual home
    let awayTeam = event.home?.name || event.home || '';  // Goalserve home = actual away
    ...
    // IMPORTANT: Goalserve inplay has home/away reversed, so we swap scores too
    let homeScore = 0, awayScore = 0;
    
    // First try info.score (most accurate real-time)
    // Format is "goalserve_home:goalserve_away" but labels are reversed
    if (event.info?.score) {
      const scores = event.info.score.split(':');
      // Swap: Goalserve home score (scores[0]) = our away score
      awayScore = parseInt(scores[0]) || 0;
      homeScore = parseInt(scores[1]) || 0;
    }
    // Then try team_info scores - swap home/away
    ...
    // Then try ss field - swap order
    else if (event.ss) {
      const scores = event.ss.split('-');
      awayScore = parseInt(scores[0]) || 0;
      homeScore = parseInt(scores[1]) || 0;
    }
    // Finally try stats.T (total score) - swap home/away
    else if (event.stats) {
  ...
  normalizeOdds(odds, sport = 'unknown') {
    ...
    for (const market of markets) {
      ...
      // IMPORTANT: Goalserve inplay has home/away reversed, so swap the assignments
      if ((marketName.includes('money line') || marketName.includes('home/away') || marketName === 'home away' || marketName === 'match winner') && !normalized.moneyline) {
        // Goalserve "Home" label = our away, Goalserve "Away" label = our home
        ...
        if ((goalserveHomeOdd && goalserveHomeOdd.suspend !== '1') || (goalserveAwayOdd && goalserveAwayOdd.suspend !== '1')) {
          normalized.moneyline = {
            home: this.parseOddsValue(goalserveAwayOdd),  // Swap: Goalserve away = our home
            away: this.parseOddsValue(goalserveHomeOdd),  // Swap: Goalserve home = our away
      ...
      // IMPORTANT: Goalserve inplay has home/away reversed
      if (isSpreadMarket && (!normalized.spread || (isPreferredSpreadMarket && !normalized._spreadIsPreferred))) {
        // Goalserve "Home" = our away, Goalserve "Away" = our home
        ...
        // Swap: use Goalserve away as our home, Goalserve home as our away
        const homeParticipants = goalserveAwayParticipants;
        const awayParticipants = goalserveHomeParticipants;
