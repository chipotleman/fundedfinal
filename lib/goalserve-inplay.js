...
// Note: Must match REST API path conventions (football not amfootball, bsktbl for basketball)
const INPLAY_ODDS_SPORT_PATHS = {
  soccer: 'soccer/inplay-odds',
  basketball: 'bsktbl/inplay-odds',
  amfootball: 'football/inplay-odds',  // Goalserve uses 'football' not 'amfootball' for odds
...
class GoalserveInplayService {
  ...
  async fetchInplayOdds(sport) {
    ...
    const oddsUrl = `https://www.goalserve.com/getfeed/${apiKey}/${oddsFeedPath}`;
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status !== 403) {
          console.error(`[Goalserve Inplay Odds] ${sport} HTTP ${response.status} - path: ${oddsFeedPath}`);
        }
        return null;
      }
      ...
      if (text.startsWith('<!DOCTYPE') || text.startsWith('<html')) {
        console.error(`[Goalserve Inplay Odds] ${sport} returned HTML instead of JSON - path: ${oddsFeedPath}`);
        return null;
      }

      const data = JSON.parse(text);
      console.log(`[Goalserve Inplay Odds] ${sport} success - path: ${oddsFeedPath}`);
      return data;
    } catch (error) {
      console.error(`[Goalserve Inplay Odds] ${sport} error: ${error.message} - path: ${oddsFeedPath}`);
      return null;
    }
  }
