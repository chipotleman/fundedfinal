...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    // Extract team names from various formats - try multiple possible field locations
    let homeTeam = event.home?.name || event.home?.n || event.home || '';
    let awayTeam = event.away?.name || event.away?.n || event.away || '';
    
    // Try team_info structure
    if ((!homeTeam || !awayTeam) && event.team_info) {
      homeTeam = homeTeam || event.team_info.home?.name || event.team_info.home?.n || '';
      awayTeam = awayTeam || event.team_info.away?.name || event.team_info.away?.n || '';
    }
    
    // Try t1/t2 structure (WebSocket format)
    if ((!homeTeam || !awayTeam) && (event.t1 || event.t2)) {
      homeTeam = homeTeam || event.t1?.name || event.t1?.n || '';
      awayTeam = awayTeam || event.t2?.name || event.t2?.n || '';
    }
    
    // Try participants array
    if ((!homeTeam || !awayTeam) && event.participants) {
      const participants = Array.isArray(event.participants) ? event.participants : Object.values(event.participants);
      if (participants.length >= 2) {
        homeTeam = homeTeam || participants[0]?.name || participants[0]?.n || '';
        awayTeam = awayTeam || participants[1]?.name || participants[1]?.n || '';
      }
    }
    ...
    // Log if we still don't have team names
    if (!homeTeam || !awayTeam) {
      console.log(`[Inplay] Missing team names for ${sport} event:`, 
        JSON.stringify({ id: event.id, home: event.home, away: event.away, t1: event.t1, t2: event.t2, participants: event.participants }, null, 2).substring(0, 500));
    }
