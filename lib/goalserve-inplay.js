...
const INPLAY_ODDS_SPORT_PATHS = {
  ...
  amfootball: 'amfootball/inplay-odds',
...
class GoalserveInplayService {
  ...
  async fetchInplayOdds(sport) {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status !== 403) { // Don't spam logs for IP whitelist issues
          console.error(`[Goalserve Inplay Odds] ${sport} HTTP ${response.status}`);
        }
        return null;
      }
      ...
      console.error(`[Goalserve Inplay Odds] ${sport} error:`, error.message);
      return null;
    }
  }
