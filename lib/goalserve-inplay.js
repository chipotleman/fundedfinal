...
class GoalserveInplayService {
  constructor() {
    ...
    // Configurable poll interval: default 2s for snappy updates, fallback to 15s if env var set
    this.pollInterval = parseInt(process.env.GOALSERVE_POLL_INTERVAL_MS || '2000', 10);
    this.events = {};
    this.rawEvents = {};
    this.lastUpdate = null;
    this.errors = new Map();
    this.backoffMultiplier = new Map(); // Track backoff per sport for 429/403 errors
  }
  ...
  async fetchFeed(sport) {
    ...
    // Note: Inplay feeds use IP whitelisting only, not API key auth
    // 403 errors mean the server IP is not whitelisted with Goalserve
    ...
    try {
      ...
      if (!response.ok) {
        // Track rate limiting (429) and access errors (403) with exponential backoff
        if (response.status === 429 || response.status === 403) {
          const currentBackoff = this.backoffMultiplier.get(sport) || 1;
          this.backoffMultiplier.set(sport, Math.min(currentBackoff * 2, 16)); // Max 16x backoff (32s at 2s base)
        }
  ...
  async fetchAllFeeds() {
    const results = {};
    // Include all sports - esports contains Ebasketball
  ...
  normalizeEvent(event, sport) {
    ...
    // Map sport feed names to display-friendly sport names
    ...
    // Determine the league/sport name for display
    // Priority: league from event > mapped sport name > raw sport
    const leagueName = event.league?.name || event.league || sportDisplayNames[sport] || sport;
