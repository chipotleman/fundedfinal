...
class GoalserveInplayService {
  ...
  normalizeOdds(odds, sport = 'unknown') {
    ...
    for (const market of markets) {
      ...
      if (isSpreadMarket && !normalized.spread) {
        ...
        // Priority: pairs where at least one side has is_main='1'
        let bestPair = null;
        let bestPairIsMain = false;
        
        for (const home of homeParticipants) {
          const homeHcap = parseFloat(home.handicap) || 0;
          // Look for away with opposite sign (or matching magnitude)
          const matchingAway = awayParticipants.find(away => {
            const awayHcap = parseFloat(away.handicap) || 0;
            // Handicaps should be opposite signs and equal magnitude
            return Math.abs(Math.abs(homeHcap) - Math.abs(awayHcap)) < 0.01;
          });
          
          if (matchingAway) {
            const isMainPair = home.is_main === '1' || matchingAway.is_main === '1';
            // Prefer main line pair, otherwise take first valid pair
            if (!bestPair || (isMainPair && !bestPairIsMain)) {
              bestPair = { home, away: matchingAway };
              bestPairIsMain = isMainPair;
            }
          }
        }
        ...
        if (!bestPair) {
          ...
          if (mainHome || mainAway) {
            bestPair = { home: mainHome, away: mainAway };
          }
        }
        ...
        if (bestPair && (bestPair.home || bestPair.away)) {
          ...
          if (sport === 'basketball') {
            console.log(`[Inplay Basketball Spread] Selected: Home ${bestPair.home?.handicap} @ ${bestPair.home?.value_eu}, Away ${bestPair.away?.handicap} @ ${bestPair.away?.value_eu}, isMain: ${bestPairIsMain}`);
          }
        }
      }
