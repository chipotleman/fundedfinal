...
class GoalserveInplayService {
  ...
  // REST API fallback for live games when inplay-odds endpoint doesn't work
  async fetchRestApiFallbackOdds(sports) {
    const apiKey = process.env.GOALSERVE_API_KEY;
    if (!apiKey) return;
    
    // Map inplay sport names to REST API schedule paths
    const REST_SCHEDULE_PATHS = {
      amfootball: ['football/nfl-shedule', 'football/fbs-shedule'],
      basketball: ['bsktbl/nba-shedule', 'bsktbl/ncaa-shedule'],
      hockey: ['hockey/nhl-shedule'],
      baseball: ['baseball/mlb_shedule']
    };
    
    for (const sport of sports) {
      const schedulePaths = REST_SCHEDULE_PATHS[sport];
      if (!schedulePaths) continue;
      
      // Find live events for this sport that need odds
      const liveEventsNeedingOdds = Object.entries(this.events).filter(([key, event]) => {
        const eventSport = SPORT_MAPPING[event.sport] || event.sport;
        return eventSport === sport && !event.odds;
      });
      
      if (liveEventsNeedingOdds.length === 0) continue;
      
      console.log(`[Goalserve REST Fallback] Fetching odds for ${liveEventsNeedingOdds.length} live ${sport} events`);
      
      // Fetch odds from REST API schedule endpoints
      for (const schedulePath of schedulePaths) {
        try {
          const url = `https://www.goalserve.com/getfeed/${apiKey}/${schedulePath}?json=1`;
          const response = await fetch(url);
          if (!response.ok) continue;
          
          const data = await response.json();
          
          // Extract matches from schedule data
          const matches = [];
          const root = data.scores || data.shedule || data.fixtures || data;
          ...
          // Match and merge odds into live events
          for (const [eventKey, event] of liveEventsNeedingOdds) {
            if (event.odds) continue; // Already has odds
            
            // Find matching game by team names (fuzzy match)
            const matchingGame = matches.find(m => {
              if (!m.odds) return false;
              const restHome = (m.hometeam?.name || m.localteam?.name || '').toLowerCase();
              const restAway = (m.awayteam?.name || m.visitorteam?.name || '').toLowerCase();
              const eventHome = (event.homeTeam || '').toLowerCase();
              const eventAway = (event.awayTeam || '').toLowerCase();
              
              // Check for partial team name matches
              return (restHome.includes(eventHome) || eventHome.includes(restHome) ||
                      restAway.includes(eventAway) || eventAway.includes(restAway)) ||
                     (restHome.includes(eventAway) || eventAway.includes(restHome) ||
                      restAway.includes(eventHome) || eventHome.includes(restAway));
            });
            
            if (matchingGame && matchingGame.odds) {
              const normalizedOdds = this.normalizeOdds(matchingGame.odds, sport, event.homeTeam, event.awayTeam);
              if (normalizedOdds) {
                this.events[eventKey].odds = normalizedOdds;
                console.log(`[Goalserve REST Fallback] Merged odds for ${event.homeTeam} vs ${event.awayTeam}`);
              }
            }
          }
        } catch (error) {
          console.error(`[Goalserve REST Fallback] Error fetching ${schedulePath}:`, error.message);
        }
      }
    }
  }
  ...
  async fetchAllFeeds() {
    ...
    // Then fetch and merge odds data from authenticated endpoints
    const sportsNeedingRestFallback = [];
      ...
      sports.map(async (sport) => {
        try {
          ...
          if (oddsData) {
            ...
            // Mark this sport for REST API fallback
            sportsNeedingRestFallback.push(sport);
          }
        } catch (error) {
          sportsNeedingRestFallback.push(sport);
        }
    ...
    // REST API fallback for sports without working inplay-odds (especially football)
    if (sportsNeedingRestFallback.length > 0) {
      await this.fetchRestApiFallbackOdds(sportsNeedingRestFallback);
    }
