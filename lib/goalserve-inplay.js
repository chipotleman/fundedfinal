...
class GoalserveInplayService {
  constructor() {
    ...
    this.pollInterval = 200; // 200ms for subsecond latency
    this.events = {};
  ...
  normalizeEvent(event, sport) {
    ...
    // Display format: Away @ Home (away on top, home on bottom)
    ...
    // Goalserve uses standard convention: format is "home:away" or home/away labels
    let homeScore = 0, awayScore = 0;
    
    // First try info.score (most accurate real-time)
    // Format is "home:away" in Goalserve
    if (event.info?.score) {
    ...
    // Then try ss field (format "home-away")
    else if (event.ss) {
