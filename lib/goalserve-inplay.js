...
class GoalserveInplayService {
  ...
  async fetchFeed(sport) {
    ...
    try {
      ...
      if (errCount >= 3) {
        const backoffMs = Math.min(300000, 10000 * Math.pow(2, Math.min(errCount - 3, 5)));
        this.circuitBreakerUntil.set(sport, Date.now() + backoffMs);
        if (errCount === 3 || errCount % 20 === 0) {
          console.warn(`[Goalserve Inplay] Circuit breaker OPEN for ${sport}: ${errCount} errors, backing off ${Math.round(backoffMs / 1000)}s`);
        }
      } else {
