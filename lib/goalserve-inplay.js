  ...
  normalizeEvent(event, sport) {
    ...
    if (event.info) {
      if (!period && event.info.period) {
        const periodLower = event.info.period.toLowerCase();
        if (periodLower.includes('overtime') || periodLower.includes('overtimer') || periodLower === 'ot') {
          period = 'OT';
        } else {
          period = event.info.period;
        }
      }
    ...
    if (event.extra) {
      if (!period && event.extra.period) {
        const periodLower = event.extra.period.toLowerCase();
        if (periodLower.includes('overtime') || periodLower.includes('overtimer') || periodLower === 'ot') {
          period = 'OT';
        } else {
          period = event.extra.period;
        }
      }
    ...
    if (!period && event.period) {
      const periodLower = event.period.toLowerCase();
      if (periodLower.includes('overtime') || periodLower.includes('overtimer') || periodLower === 'ot') {
        period = 'OT';
      } else {
        period = event.period;
