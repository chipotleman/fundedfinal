...
class GoalserveInplayService {
  ...
  normalizeEvent(event, sport) {
    ...
    if (event.timer) {
      ...
      // Format: "Q2 04:32" or "3rd 12:45" or just the time remaining
      let period = '';
      if (timer.q) period = `Q${timer.q}`;
      else if (timer.period) period = timer.period;
    ...
    if (!displayClock && event.extra?.period) {
      displayClock = event.extra.period;
    }
    if (!displayClock && event.info?.period) {
      displayClock = event.info.period;
    }
