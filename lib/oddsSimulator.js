export function simulateOddsMovement(games) {
  return games.map(game => {
    ...
    // Much higher probability of changes for more engaging experience
    if (Math.random() < 0.7) {
...
function simulateSpreadMovement(spread) {
  ...
  const lineMove = (Math.random() < 0.3) ? (Math.random() < 0.5 ? 0.5 : -0.5) : 0;
  ...
  if (lineMove !== 0) {
    ...
    // Always adjust odds when no line movement
    newSpread.away = {
      ...spread.away,
      odds: adjustOdds(spread.away.odds, true),
      moved: null
    };
    newSpread.home = {
      ...spread.home,
      odds: adjustOdds(spread.home.odds, true),
      moved: null
    };
  }
...
function simulateTotalMovement(total) {
  ...
  if (lineMove !== 0) {
    ...
    newTotal.over = {
      ...total.over,
      odds: adjustOdds(total.over.odds, true),
      moved: null
    };
    newTotal.under = {
      ...total.under,
      odds: adjustOdds(total.under.odds, true),
      moved: null
    };
  }
...
function simulateMoneylineMovement(moneyline) {
  ...
  const awayChange = adjustMoneylineOdds(moneyline.away, true);
  const homeChange = adjustMoneylineOdds(moneyline.home, true);
...
function adjustOdds(odds, forceChange = false) {
  if (typeof odds !== 'number') return odds;
  
  // 80% chance to change odds (was 30%)
  if (!forceChange && Math.random() < 0.2) return odds;
  
  // Random change of 5 or 10 points
  const changeAmount = Math.random() < 0.7 ? 5 : 10;
  ...
  return Math.max(-250, Math.min(250, newOdds));
}

function adjustMoneylineOdds(odds, forceChange = false) {
  if (typeof odds !== 'number') return { odds, moved: null };
  
  // 70% chance to change (was 40%)
  if (!forceChange && Math.random() < 0.3) return { odds, moved: null };
