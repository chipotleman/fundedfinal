...
function parseUpdateMessage(data) {
  try {
    ...
    // Per Goalserve docs: stats.a contains goals [home, away]
    if (data.stats?.a && Array.isArray(data.stats.a) && data.stats.a.length >= 2) {
      homeScore = parseInt(data.stats.a[0]) || 0;
      awayScore = parseInt(data.stats.a[1]) || 0;
    }
    // Try score string format "3-2"
    else if (data.ss && typeof data.ss === 'string') {
