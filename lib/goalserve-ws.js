...
function parseAvailableEventWithScores(evt, sport) {
  try {
    ...
    // Try score string format "3-2" or "3:2"
    if (evt.ss && typeof evt.ss === 'string') {
    ...
    // Try t1/t2 score format
    else if (evt.t1?.s !== undefined || evt.t2?.s !== undefined) {
      homeScore = parseInt(evt.t1?.s) || 0;
      awayScore = parseInt(evt.t2?.s) || 0;
    }
    // Try sc array (periods)
    else if (Array.isArray(evt.sc)) {
    ...
    // Debug log if scores found
    if ((homeScore > 0 || awayScore > 0) && !liveDataStore._debuggedAvlScores) {
      console.log('[Goalserve WS] AVL Score debug:', {
        eventId: evt.id,
        homeScore, awayScore,
        ss: evt.ss,
        t1s: evt.t1?.s,
      ...
      liveDataStore._debuggedAvlScores = true;
    }
...
function parseUpdateMessage(data) {
  try {
    ...
    // Goalserve sends scores in multiple possible formats:
    // - data.ss = "3-2" (score string)
    // - data.sc = [{id: "1", home: "1", away: "0"}, ...] (by period)
    // - data.t1.s / data.t2.s (team scores)
    // - data.s = {home: 3, away: 2}
    let homeScore = 0;
    let awayScore = 0;
    
    // Try score string format "3-2"
    if (data.ss && typeof data.ss === 'string') {
    ...
    // Try t1/t2 team score format
    else if (data.t1?.s !== undefined || data.t2?.s !== undefined) {
      homeScore = parseInt(data.t1?.s) || 0;
      awayScore = parseInt(data.t2?.s) || 0;
    }
    // Try s.home/s.away format
    else if (data.s?.home !== undefined || data.s?.away !== undefined) {
      homeScore = parseInt(data.s?.home) || 0;
      awayScore = parseInt(data.s?.away) || 0;
    }
    // Try sc array (sum all periods)
    else if (Array.isArray(data.sc)) {
      data.sc.forEach(period => {
        homeScore += parseInt(period.home) || 0;
        awayScore += parseInt(period.away) || 0;
      });
    }
    // Fallback to a array (original format)
    ...
    rawOdds.forEach(odd => {
      ...
      // Moneyline detection - various market IDs and names
      // Common IDs: 2 (1x2), 23 (money line), 1 (match winner)
      const isMoneyline = marketId === 2 || marketId === 23 || marketId === 1 ||
        marketName.includes('money') || marketName.includes('winner') || 
        marketName.includes('1x2') || marketName.includes('match result');
      
      if (isMoneyline && !parsedOdds.moneyline.home) {
        outcomes.forEach(o => {
          const name = (o.name || '').toLowerCase();
          if (name === '1' || name.includes('home') || name === 'h') {
            parsedOdds.moneyline.home = o.value;
          } else if (name === '2' || name.includes('away') || name === 'a') {
      ...
      // Common IDs: 4 (handicap), 3 (asian handicap), 52 (spread)
      const isSpread = marketId === 4 || marketId === 3 || marketId === 52 ||
        marketName.includes('spread') || marketName.includes('handicap') ||
        marketName.includes('run line') || marketName.includes('puck line');
      
      if (isSpread && !parsedOdds.spread.home) {
        const line = parseFloat(odd.handicap) || 0;
        outcomes.forEach(o => {
          const name = (o.name || '').toLowerCase();
          if (name === '1' || name.includes('home') || name === 'h') {
            parsedOdds.spread.home = { line: line, odds: o.value };
          } else if (name === '2' || name.includes('away') || name === 'a') {
            parsedOdds.spread.away = { line: -line, odds: o.value };
          }
        });
      }
      
      // Totals/Over-Under detection
      // Common IDs: 5 (over/under), 45 (totals)
      const isTotal = marketId === 5 || marketId === 45 ||
        marketName.includes('total') || marketName.includes('over') ||
        marketName.includes('under') || marketName.includes('o/u');
      
      if (isTotal && !parsedOdds.total.over) {
        const line = parseFloat(odd.handicap) || 0;
        outcomes.forEach(o => {
          const name = (o.name || '').toLowerCase();
          if (name.includes('over') || name === 'o') {
            parsedOdds.total.line = line;
            parsedOdds.total.over = o.value;
          } else if (name.includes('under') || name === 'u') {
  ...
  try {
    ...
    switch (messageType) {
      case 'avl': {
        ...
        // Some Goalserve feeds include score data in the avl message's evts array
        if (data.evts && data.evts.length > 0) {
          // Debug: Log first event structure
          if (!liveDataStore._debuggedAvlEvent) {
            console.log('[Goalserve WS] AVL Event structure sample:', JSON.stringify(data.evts[0], null, 2));
            liveDataStore._debuggedAvlEvent = true;
          }
          
          data.evts.forEach(evt => {
            // Always parse events from avl to capture live data
