...
// Periodic reconnection as fallback if UPDT messages aren't being received
// Only reconnects if no updates received in the last 2 minutes
setInterval(async () => {
  if (liveDataStore.connectionStatus === 'connected' && liveDataStore.activeSports.size > 0) {
    const timeSinceUpdate = Date.now() - (liveDataStore.lastUpdate || 0);
    
    // Only reconnect if stale (no updates in 2 minutes)
    if (timeSinceUpdate > 120 * 1000) {
      console.log('[Goalserve WS] Stale data detected - reconnecting to get fresh scores...');
      const sports = Array.from(liveDataStore.activeSports);
      
      // Clear old subscriptions to resubscribe with correct format
      liveDataStore.subscribedEvents.clear();
      
      disconnect();
      await connect(sports);
      console.log('[Goalserve WS] Refresh complete');
    }
  }
