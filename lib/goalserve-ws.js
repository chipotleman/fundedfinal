...
// Periodic reconnection only if connection is stale (no updates in 5 minutes)
// Avoid aggressive reconnects that destabilize the connection
setInterval(async () => {
  if (liveDataStore.activeSports.size > 0) {
    const timeSinceUpdate = Date.now() - (liveDataStore.lastUpdate || 0);
    
    // Only reconnect if stale (no updates in 5 minutes) or connection failed
    if (timeSinceUpdate > 300 * 1000 || liveDataStore.connectionStatus === 'failed') {
      console.log('[Goalserve WS] Stale data or failed connection - attempting reconnect...');
      const sports = Array.from(liveDataStore.activeSports);
