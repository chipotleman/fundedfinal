...
function parseUpdateMessage(data) {
  try {
    ...
    const comments = (data.cms || []).map((cm) => {
      const text = cm.n || '';
      const minute = cm.tm || 0;
      const player = cm.p || '';
      const textHash = text.substring(0, 30).replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
      const playerHash = player.substring(0, 10).replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
      const deterministicId = cm.id || `${eventId}_${minute}_${textHash}_${playerHash}`;
      return {
        id: deterministicId,
        type: cm.mt || 'default',
        minute: minute || null,
