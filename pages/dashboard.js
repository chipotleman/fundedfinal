...
export default function Dashboard() {
  ...
  return (
    <div className="bg-black text-white min-h-screen font-mono pt-20">
      <TopNavbar
        bankroll={bankroll}
              ...
              return (
                <div key={game.id} className="rounded-lg border border-zinc-700 bg-zinc-900 overflow-hidden flex">
                  {[team1, team2].map((team, idx) => {
                    const selected = selectedBet?.team === team.trim();
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className={`mt-1 text-sm text-center ${selected ? 'text-black' : 'text-green-300'}`}>{team.trim()}</p>
                        <p className={`text-xs ${selected ? 'text-black' : 'text-gray-400'}`}>Odds: {odds && !isNaN(odds) ? decimalToAmerican(parseFloat(odds)) : 'N/A'}</p>
                        <p className={`text-[10px] ${selected ? 'text-black' : 'text-gray-500'}`}>{new Date(game.game_time).toLocaleString()}</p>
                      </div>
                    );
                  })}
                </div>
