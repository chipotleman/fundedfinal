    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl">
        <div className="flex items-center justify-between p-4 border-b border-gray-800">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
            <span className="text-red-400 font-medium">LIVE BATTLE</span>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-gray-800 rounded-lg transition">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            <p className="text-4xl font-bold text-purple-400">${battle.potSize}</p>
            <p className="text-sm text-gray-400 mt-1">Prize Pool</p>
          </div>

          <div className="flex items-center justify-between gap-8">
            <div className="flex-1 text-center">
              <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-purple-600 to-indigo-800 flex items-center justify-center overflow-hidden mb-3">
                {battle.user1?.avatar ? (
                  <img src={battle.user1.avatar} alt="" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-2xl font-bold">{battle.user1?.username?.charAt(0)?.toUpperCase()}</span>
                )}
              </div>
              <p className="font-semibold text-lg">{battle.user1?.username}</p>
              <p className="text-sm text-gray-400">{battle.user1?.battleWins || 0}W - {battle.user1?.battleLosses || 0}L</p>
              <div className="mt-3 p-3 bg-gray-800 rounded-xl">
                <p className="text-xs text-gray-400">Balance</p>
                <p className="text-xl font-bold">${parseFloat(battle.user1?.balance || 0).toFixed(2)}</p>
                <p className={`text-sm font-medium ${parseFloat(battle.user1?.pnl || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {parseFloat(battle.user1?.pnl || 0) >= 0 ? '+' : ''}{battle.user1?.pnlPercent || '0.0'}%
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 text-center">
              <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-purple-600 to-indigo-800 flex items-center justify-center overflow-hidden mb-3">
                {battle.user2?.avatar ? (
                  ...
                  <span className="text-2xl font-bold">{battle.user2?.username?.charAt(0)?.toUpperCase()}</span>
                )}
              </div>
              <p className="font-semibold text-lg">{battle.user2?.username}</p>
              <p className="text-sm text-gray-400">{battle.user2?.battleWins || 0}W - {battle.user2?.battleLosses || 0}L</p>
              <div className="mt-3 p-3 bg-gray-800 rounded-xl">
                <p className="text-xs text-gray-400">Balance</p>
                <p className="text-xl font-bold">${parseFloat(battle.user2?.balance || 0).toFixed(2)}</p>
                <p className={`text-sm font-medium ${parseFloat(battle.user2?.pnl || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {parseFloat(battle.user2?.pnl || 0) >= 0 ? '+' : ''}{battle.user2?.pnlPercent || '0.0'}%
                </p>
              </div>
            </div>
          ...
          <div className="mt-6">
            ...
            <p className="text-center text-xs text-gray-400 mt-2">{Math.round(battle.progressPercent || 0)}% complete</p>
          </div>
        </div>
      </div>
    </div>
