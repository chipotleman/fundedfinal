...
export default function SpectatePage() {
  ...
  const fetchBattle = useCallback(async () => {
    ...
    try {
      ...
      const found = (data.battles || []).find((b) => b.id === id);
      if (found) {
        setBattle(found);
        setNotFound(false);
      } else if (loadingBattle) {
        setNotFound(true);
      }
    } catch {
  ...
  // Poll battle every 5s, messages every 3s while the page is open.
  useEffect(() => {
    if (!id) return;
    const t1 = setInterval(fetchBattle, 5000);
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-bold text-sm">Spectator chat</span>
                  </div>
